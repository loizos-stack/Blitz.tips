 ...
 */
export async function uploadMedia(token: string, bytes: Buffer, mime: string): Promise<{ id: string } | { error: string }> {
  ...
  try {
    ...
    if (!id) return { error: "Media upload returned no id." };
    return { id };
  } catch (e) {
 ...
 */
export async function postTweet(opts: { text: string; media?: { bytes: Buffer; mime: string } }): Promise<PostResult> {
  ...
  if (opts.media) {
    const up = await uploadMedia(auth.token, opts.media.bytes, opts.media.mime);
